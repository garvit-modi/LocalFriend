...
class TinyDB     //preferences = PreferenceManager.getDefaultSharedPreferences(appContext);
     ...
     */
    fun putImage(theFolder: String?, theImageName: String?, theBitmap: Bitmap?): Boolean {
    ...
    fun getListDouble(key: String?): ArrayList<Double> {
        val myList = TextUtils.split(preferences.getString(key, ""), "â€šâ€—â€š")
        val arrayToList = ArrayList(Arrays.asList(*myList))
    ...
    fun getString(key: String): String? {
        return preferences.getString(key, if (key.equals("user_id", ignoreCase = true)) "0" else "")
    }

    fun getSportKey(key: String?): String? {
        return preferences.getString(key, "CRICKET")
    }
